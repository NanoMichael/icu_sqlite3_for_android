...
public class MainActivity extends AppCompatActivity {
	...
	*/
	...
	public void run_load_extension() {
		SQLiteDatabase.deleteDatabase(DB_PATH);
		MyHelper helper = new MyHelper(this);
		SQLiteDatabase db = helper.getWritableDatabase();
		Log.e(TAG, "load spellfix");
		...
		c = db.rawQuery("select word from spellfix where word match 'frustate'", null);
		while (c.moveToNext()) {
			Log.d(TAG, "search spell fix frustate : " + c.getString(0));
		}
		c.close();
		helper.close();
	}

	public void run_icu_test() {
		SQLiteDatabase.deleteDatabase(DB_PATH);
		MyHelper helper = new MyHelper(this);
		SQLiteDatabase db = helper.getWritableDatabase();
		Log.e(TAG, "create fts table with icu");
		db.execSQL("create virtual table icu_fts using fts4(tokenize=icu)");
		Log.e(TAG, "create fts table success");
		db.execSQL("insert into icu_fts values('这是一段中文呀')");
		db.execSQL("insert into icu_fts values('北京欢迎你！')");
		db.execSQL("insert into icu_fts values('中国欢迎你！')");
		Cursor c = db.rawQuery("select * from icu_fts where icu_fts match '中文'", null);
		while (c.moveToNext()) {
			Log.d(TAG, "search 中文 : " + c.getString(0));
		}
		c.close();
		c = db.rawQuery("select * from icu_fts where icu_fts match '欢迎'", null);
		while (c.moveToNext()) {
			Log.d(TAG, "search 欢迎 : " + c.getString(0));
		}
		c.close();
		helper.close();
	}
